    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1600px] mx-auto text-slate-900">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full bg-slate-50 text-slate-900 text-xs rounded-full pl-9 pr-4 py-2 border border-slate-200 focus:outline-none focus:border-[#2563EB]"
          />
        </div>
      ...
      <div className="space-y-4">
        {filteredInspections.map(insp => (
          <div
            key={insp.id}
            className="bg-white p-5 rounded-2xl border border-slate-100 shadow-xs hover:shadow-md transition space-y-3"
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-slate-100 pb-3">
              <div className="flex items-center gap-3">
                <span className="font-mono font-bold text-[#2563EB] text-sm">{insp.assetCode}</span>
                <span className="font-bold text-slate-900 text-xs">{insp.assetName}</span>
                <span className="text-[10px] bg-slate-100 px-2.5 py-0.5 rounded-full text-slate-600 border border-slate-200/80 font-bold">
                  {insp.issueCategory}
                </span>
              </div>
            ...
            <p className="text-slate-700 text-xs leading-relaxed font-medium">{insp.notes}</p>

            {insp.photos && insp.photos.length > 0 && (
              <div className="flex gap-3 pt-1">
                {insp.photos.map((url, idx) => (
                  <img key={idx} src={url} alt="Inspeksi Foto" className="w-32 h-24 object-cover rounded-2xl border border-slate-200 shadow-xs" />
                ))}
              </div>
            )}
            ...
            <div className="flex items-center justify-between text-[11px] text-slate-500 pt-2 border-t border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span>Petugas: <strong className="text-slate-900">{insp.inspectorName}</strong> ({insp.inspectorRole})</span>
              </div>

              {insp.actionTaken && (
                <div className="text-[#2563EB] font-bold">Tindakan: {insp.actionTaken}</div>
              )}
            </div>
          </div>
        ))}
