    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1600px] mx-auto text-slate-900 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/80 pb-5">
        ...
        <button
          ...
          className="flex items-center gap-2.5 bg-[#2563EB] text-white font-black text-sm px-6 py-3.5 rounded-full hover:bg-[#1D4ED8] transition shadow-md shadow-blue-500/25 shrink-0 self-start md:self-auto"
        >
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-white p-6 rounded-xl border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div className="space-y-3">
              {users.map(usr => (
                <div key={usr.id} className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <img src={usr.avatar} alt={usr.name} className="w-10 h-10 rounded-full object-cover border border-blue-200 shadow-xs shrink-0" />
                    <div className="min-w-0">
                      <div className="font-extrabold text-slate-900 text-sm truncate flex items-center gap-1.5">
                        <span>{usr.name}</span>
                        {usr.id === currentUser.id && (
                          <span className="text-[10px] bg-blue-100 text-[#2563EB] font-bold px-2 py-0.5 rounded-full">Anda</span>
                        )}
                      </div>
                      <div className="text-xs text-slate-600 font-medium truncate">{usr.email}</div>
                      <div className="text-xs text-slate-500 font-semibold truncate">{usr.department}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className="p-1.5 bg-white hover:bg-amber-50 hover:text-amber-700 text-slate-600 rounded-lg border border-slate-200 transition"
                    ...
                    <button
                      ...
                      className={`p-1.5 bg-white rounded-lg border border-slate-200 transition ${
                        usr.id === currentUser.id ? 'opacity-40 cursor-not-allowed text-slate-300' : 'hover:bg-rose-50 hover:text-rose-600 text-slate-600'
                      }`}
