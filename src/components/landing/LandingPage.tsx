...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  return (
    <div className={`h-screen w-full font-sans flex flex-col items-center justify-center p-6 sm:p-10 relative overflow-hidden transition-colors duration-300 selection:bg-[#2563EB] selection:text-white ${isDarkMode ? 'bg-[#0B0F17] text-white' : 'bg-[#F4F5F7] text-slate-900'
      }`}>
      {/* Ambient background glowing lights */}
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full blur-[160px] pointer-events-none transition-colors duration-300 ${isDarkMode ? 'bg-blue-600/15' : 'bg-blue-500/10'
        }`}></div>
      <div className={`absolute bottom-10 right-10 w-[450px] h-[450px] rounded-full blur-[130px] pointer-events-none transition-colors duration-300 ${isDarkMode ? 'bg-emerald-600/10' : 'bg-emerald-500/10'
        }`}></div>
      ...
      <header className="absolute top-6 left-6 right-6 sm:top-8 sm:left-10 sm:right-10 z-20 flex items-center justify-between">
        {/* Top Left Logo */}
        <div className={`p-2 sm:p-2.5 rounded-xl border shadow-md transition-colors ${isDarkMode ? 'bg-white/95 border-white/20' : 'bg-white border-slate-200/80 shadow-xs'
          }`}>
        ...
        <div className="flex items-center gap-3">
          {onToggleDarkMode && (
            <button
              ...
              className={`p-3 rounded-xl border transition-all backdrop-blur-md cursor-pointer flex items-center justify-center shadow-md hover:scale-105 ${isDarkMode
      ...
      <main className="w-full max-w-4xl mx-auto z-10 text-center flex flex-col items-center justify-center space-y-6 my-auto">
        ...
        <div className="space-y-4 max-w-3xl mx-auto text-center flex flex-col items-center">
          ...
          <p className={`text-sm sm:text-base lg:text-lg font-medium max-w-2xl mx-auto leading-relaxed text-center pt-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'
            }`}>
        ...
        <div className={`pt-8 border-t grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8 text-center w-full max-w-4xl mx-auto transition-colors ${isDarkMode ? 'border-slate-800/80' : 'border-slate-300/80'
          }`}>
